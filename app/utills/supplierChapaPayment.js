...
// Get credentials from environment variables for supplier-specific Chapa account
const CHAPA_PUBLIC_KEY = Constants.expoConfig?.extra?.supplierChapaPublicKey || '';
...
export const generateTxRef = () => {
    ...
    return `STX-${uniqueId.substring(0, 4)}-${uniqueId.substring(4, 8)}-${uniqueId.substring(8, 12)}`;
};

/**
 * Initialize a Chapa payment transaction for supplier orders
...
export const initializePayment = async (paymentData) => {
    try {
        ...
        console.log('Initializing Supplier Chapa payment with data:', JSON.stringify(paymentData, null, 2));
        ...
        console.log('Supplier Chapa payment initialized successfully:', response.data);
        return response.data;
    } catch (error) {
        console.error('Supplier Chapa payment initialization error:',
        ...
        // For demonstration purposes only - simulate success response in development
        if (process.env.NODE_ENV === 'development' || CHAPA_SECRET_KEY.includes('TEST')) {
            console.log('DEV/TEST MODE: Simulating successful payment initialization');
            return {
                status: 'success',
                message: 'Test mode: Checkout URL created',
                data: {
...
/**
 * Verify a Chapa payment transaction for supplier orders
