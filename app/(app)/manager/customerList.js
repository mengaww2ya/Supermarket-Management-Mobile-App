...
export default function CustomerList() {
  ...
  // Random profile images for visual enhancement
  const avatars = [
  ...
  // Enhanced customer data with additional properties for UI
  const enhancedCustomers = customer.map((item, index) => ({
    ...item,
    ...
    lastPurchase: getRandomDate(30),
    memberSince: getRandomDate(365)
  }));

  // Generate a random date within the past X days
  function getRandomDate(daysAgo = 60) {
    const date = new Date();
      ...
      >
        ...
        <View className="flex-row justify-between mb-3 pt-2">
          <View className="items-center bg-white p-2 rounded-lg shadow-sm border border-gray-100 flex-1 mx-1">
            ...
            <Text className="font-bold text-gray-800 text-lg">{enhancedCustomers.length}</Text>
          </View>
          
          <View className="items-center bg-white p-2 rounded-lg shadow-sm border border-gray-100 flex-1 mx-1">
            <Text className="text-gray-500 text-xs mb-1">Active</Text>
            <Text className="font-bold text-gray-800 text-lg">
              {enhancedCustomers.filter(c => c.status === 'active').length}
            </Text>
          </View>
          ...
          <View className="items-center bg-white p-2 rounded-lg shadow-sm border border-gray-100 flex-1 mx-1">
            ...
            <Text className="font-bold text-gray-800 text-lg">
              {enhancedCustomers.filter(c => c.status === 'new').length}
            </Text>
          </View>
        </View>
