...
import { Frozenproducts } from "../global/data.js";
import Footer from "../subscrean/foter.js";
import Ionicons from "react-native-vector-icons/Ionicons";

export default function FrozenProductsList({ navigation }) {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedImage, setSelectedImage] = useState(null);
  const fadeAnim = useState(new Animated.Value(0))[0];

  // Filter only active frozen products
  const activeProducts = Frozenproducts.filter(
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-100 px-2">
      ...
      <View className="flex-row items-center justify-between bg-yellow-300 px-4 py-3">
        <Text className="text-lg font-bold text-green-600">Frozen Products</Text>
